  ...
  <div class="user-appointments-wrapper">
    ...
    <div class="appointments-list" v-if="appointmentsFiltered.length > 0 && !loading">
      ...
      >
        ...
        {{
          new Date(appointment.startSlot).toLocaleDateString(locale, {
            weekday: "long",
            day: "2-digit",
            month: "2-digit",
            year: "numeric",
        ...
        <template #footer>
          {{ t("userAppointmentsView.appointmentsList.price") }}: {{ appointment.minPrice }}zł
          <span v-if="appointment.minPrice !== appointment.maxPrice"
            >- {{ appointment.maxPrice }}zł</span
          >
        </template>
      </n-card>
    </div>
...
import { createRequestErrorMessage } from "@/utils.js";
export default {
  components: {
    NTag,
    CustomLoader,
  ...
  setup() {
    ...
    const loadAppointments = async () => {
      try {
        ...
        message.error(`Nie udało się pobrać wizyt - ${createRequestErrorMessage(error)}`);
      }
    };
